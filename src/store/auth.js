...
export default {
    ...
    actions: {
        async getRegisterData({ commit, dispatch }, payload) {
            const APIkey = "AIzaSyCt5dlK0msa2uLZsVJQZE6tfGc69ROLTDU";
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                await axios.put(
                    `https://final-test-vue-e9777-default-rtdb.firebaseio.com/user/${payload.userId}.json?auth=${state.token}`,
                    payload
                );
        ...
        async uploadProfileImage({ state, commit }, file) {
            ...
            const storageRef = `https://final-test-vue-e9777-default-rtdb.firebaseio.com/user/${userId}/imageLink.json?auth=${state.token}`;
        ...
        async updateUserData({ commit, state }, payload) {
            try {
                ...
                await axios.put(
                    `https://final-test-vue-e9777-default-rtdb.firebaseio.com/user/${state.userLogin.userId}.json?auth=${state.token}`,
                    payload
                );
        ...
        async getUser({ commit }, userId) {
            try {
                const { data } = await axios.get(
                    `https://final-test-vue-e9777-default-rtdb.firebaseio.com/user/${userId}.json`
                );
                if (data) {
