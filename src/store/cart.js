    ...
    actions: {
        async fetchCartData({ commit, rootState }) {
            ...
            try {
                const { data } = await axios.get(
                    `https://final-test-vue-e9777-default-rtdb.firebaseio.com/cart/${userId}.json`
                );
        ...
        async addProductToCart({ commit, state, rootState, dispatch }, product) {
            ...
            if (existingProduct) {
                ...
                try {
                    await axios.put(
                        `https://final-test-vue-e9777-default-rtdb.firebaseio.com/cart/${userId}/${product.id}.json`,
                        newProductData
                    );
        ...
        async removeProductFromCart({ commit }, productId) {
            ...
            try {
                await axios.delete(
                    `https://final-test-vue-e9777-default-rtdb.firebaseio.com/cart/${userId}/${productId}.json`
                );
        ...
        async placeOrder({ state, rootState, commit }) {
            ...
            try {
                ...
                for (const product of state.cart) {
                    ...
                    await axios.put(
                        `https://final-test-vue-e9777-default-rtdb.firebaseio.com/history/${userId}/${product.id}.json`,
                        orderData
                    );
                }
